...
terraform output -state=./terraform-tfstate/terraform.tfstate -json | jq -r '.ops_manager_ssh_private_key.value' > opsman
echo "Set proper permission to key."
chmod 700 opsman
echo "Upload cpi to opsman vm."
scp -o StrictHostKeyChecking=no -i opsman ./bosh-cpi-validate-release/bosh-azure-cpi ubuntu@${OPSMAN_DOMAIN_OR_IP_ADDRESS}:/home/ubuntu/cpi
